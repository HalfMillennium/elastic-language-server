...
const queryEndpointSearch = async (searchTerm = '', tablePage = 0, tableSort = ['_score']) => {
    ...
    const requestBody = esb
        ...
        .missing(1))
        .boostMode('sum'));
    const result = await axios_1.default.post(BASE_DB_URL, requestBody.toJSON());
    return result;
    //return queryElasticsearch('search-dataset/_search', requestBody.toJSON());
};
const queryFieldSearch = async (searchTerm = '') => {
    searchTerm = searchTerm.trim();
    console.log("searchTerm:", searchTerm);
    const requestBody = esb
        .requestBodySearch()
        .size(50)
        .query(esb.boolQuery().should([
    ...
    const result = await axios_1.default.post(BASE_FI_URL, requestBody.toJSON());
    return result;
...
connection.onCompletion(async (_textDocumentPosition) => {
    ...
    let value = '' + ((_a = documents === null || documents === void 0 ? void 0 : documents.get(_textDocumentPosition.textDocument.uri)) === null || _a === void 0 ? void 0 : _a.getText());
    let pattern = /\b(\w+)\W*$/g;
